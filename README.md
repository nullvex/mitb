...
The user provides a source file (hidden content)
The user provides a local directory or direct paths of images, music, or videos
The user provides a passphrase or encoded source file as a private key

MITB shards the source file, encrypts the shards with the passphrase as a salt and then embeds the encrypted shards into images, music, or videos as indicated by the user

MITB can then store the files locally or distribute them to any number of online storage targets. 

MITB returns an index file that has all the hashes and locations necessary to communicate the decrypt information to a person/thing.  This can be then stored in a secrets manager and used in conjunction with MITB to fetch and decrypt the multi-file/sharded/encrypted images, music, video and extract the file/message/string, etc.

Possibilities: 
  - Add a flag to randomly generate images, music, or videos on the fly from the mediagen project. 
  - Add a backup feature explicitly meant for packaging the index file in an passphrase pprotected archive for transport or to external storage media
...
https://medium.com/swlh/lsb-image-steganography-using-python-2bbbee2c69a2#:~:text=LSB%20Steganography%20is%20an%20image,a%202D%20array%20of%20pixels.
